...
translateCodata (L loc c@(CodataDecl {})) = unitOL dataDecl `mappend` setters
  where dataDecl = L loc . TyClD
        ...
        dataDefn = HsDataDefn { dd_ND      = DataType
                              ...
                              , dd_derivs  = noLoc [] }
        record = ConDeclH98 { con_name    = mkConstructorName (tccdLName c)
                            , con_qvars   = Nothing
                            , con_cxt     = Nothing
        ...
        fields = fmap (\(L loc d) ->
                       L loc ( ConDeclField
                             { cd_fld_names = fmap (\n -> noLoc (FieldOcc n PlaceHolder))
                                                   (dest_names d)
                             , cd_fld_type  = getLHsInstDeclHead . dest_type $ d
                             , cd_fld_doc = Nothing }))
                      (cdd_dests . tccdCodataDefn $ c)
        setters = mapOL mkSetter . toOL . cdd_dests . tccdCodataDefn $ c
        num_fields = length . cdd_dests . tccdCodataDefn $ c
        mkSetter (L _ d) = noLoc . ValD
                         $ mkFunBind (mkSetterName . head . dest_names $ d)
                                     []
