...
freshen l =
  do { ps <- getPState
     ...
     ; return (prependRdrName ("n" ++ show n ++ "_") varName l)
     }

freshVar :: P (Located RdrName)
freshVar = freshen . noLoc . mkVarUnqual . fsLit $ "x"

prependRdrName :: String -> NameSpace -> Located RdrName -> Located RdrName
prependRdrName s' space (L l s) = L l $
  case s of
...
translateCodata :: LTyClDecl GhcPs -> OrdList (LHsDecl GhcPs)
translateCodata (L loc c@(CodataDecl {})) = unitOL dataDecl `mappend` setters
  where dataDecl = L loc . TyClD
        ...
        record = ConDeclH98 { con_name    = consName
                            , con_qvars   = Nothing
                            , con_cxt     = Nothing
                            , con_details = RecCon (noLoc fields)
                            , con_doc     = Nothing }
        ...
        setters = toOL . mapWithIndex mkSetter . cdd_dests . tccdCodataDefn $ c
        ...
        mkSetter i (L _ d) = noLoc . ValD . unLoc
                           $ mk_easy_FunBind noSrcSpan
                                             ...
                                             (foldlWithIndex (\j c t ->
                                                               let t' = if i == j
                                                                        then nlHsVar varX
                                                                        else nlHsApp (nlHsVar . unLoc . mkDestName . head . dest_names . unLoc $ dests !! j)
                                                                                     (nlHsVar varD)
                                                               in nlHsApp c t')
                                                             (nlHsVar . unLoc $ consName)
                                                             dests)
translateCodata (L loc _other) = unitOL . L loc . TyClD $ _other
...
data FCocase = FCocase (FCopattern, Either (LHsExpr GhcPs) FCocase)
...
flattenCocase (Cocase coalts) =
  case coalts of
    ...
    ((QDest h QHead,u):coalts) ->
      Right . FCocase (FQDest h,Left u) <$> flattenCocase (Cocase coalts)
    ...
    ((q,u):coalts) ->
      case unplugCopattern q of
        ...
        (qrest,Just q') -> do { coalts' <- flattenCocase (Cocase coalts)
                              ; u' <- flattenCocase (Cocase [(qrest,u)])
                              ; return . Right $ FCocase (q',Right $ FCocase (FQHead,u') coalts') coalts'
                              }
        _ -> panic "recursive cases not implemented"

translateFCocase :: Either (LHsExpr GhcPs) FCocase -> P (LHsExpr GhcPs)
translateFCocase (Left u) = return u
translateFCocase (Right (FCocase (fq,u) def)) =
  case fq of
    FQHead -> translateFCocase u
    FQDest h -> panic "translating destructors incomplete"
    FQPat  p -> freshVar >>= \x -> panic "translating pattersn incomplete"
       -- mkHsLam [noLoc . VarPat $ x]
       --         (nlHsCase (noLoc . HsVar $ x) matches)
                 -- panic "oops not done implementing copattens."
