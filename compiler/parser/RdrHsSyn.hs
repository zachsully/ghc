...
---------------------------------------------------
-- sudo fresh name generation in the parser monad
freshen :: Located RdrName -> P (Located RdrName)
freshen l =
  do { ps <- getPState
     ; let n = u_num ps
     ; setPState (ps { u_num = n+1 })
     ; return (pendRdrName (show n) False varName l)
     }

freshVar :: String -> P (Located RdrName)
freshVar = freshen . noLoc . mkVarUnqual . fsLit

pendRdrName :: String -> Bool -> NameSpace -> Located RdrName -> Located RdrName
pendRdrName s' preOrPost space (L l s) = L l $
  case s of
    Unqual n -> Unqual (addS' n)
    Qual m n -> Qual m (addS' n)
    Orig m n -> Orig m (addS' n)
    Exact  n -> Exact  (n { n_occ = addS' (n_occ n) })
  where addS' n = n { occNameSpace = space
                    , occNameFS = if preOrPost
                                  then appendFS (fsLit s') (occNameFS n)
                                  else appendFS (occNameFS n) (fsLit s') }


mkConstructorName :: Located RdrName -> Located RdrName
mkConstructorName = pendRdrName "Mk" True dataName

-- these functions are generated by translating codata declarations
mkSetterName :: Located RdrName -> Located RdrName
mkSetterName = pendRdrName "set_" True varName
...
mkTyCodata
  ...
  -> [LDestDecl GhcPs]
  -> P (LTyClDecl GhcPs)
mkTyCodata loc (L _ (mcxt, tycl_hdr)) data_dests
  = do { (tc, tparams, fixity, _) <- checkTyClHdr False tycl_hdr
       ; tyvars <- checkTyVarsP (text "error parsing tyvars") equalsDots tc tparams
       ; defn <- mkCodataDefn mcxt data_dests
       ; return (L loc (CodataDecl { tccdLName      = tc
                                   , tccdTyVars     = tyvars
                                   , tccdCodataDefn = defn
                                   , tccdFixity     = fixity }))
       }
...
mkCodataDefn mcxt data_dests
  = do { checkDatatypeContext mcxt
       ; let cxt = fromMaybe (noLoc []) mcxt
       ; return (HsCodataDefn { cdd_ctxt  = cxt
                              , cdd_dests = data_dests }) }

mkDestDecl :: [Located RdrName]
           -> LHsSigType GhcPs
           -> DestDecl GhcPs
mkDestDecl names ty = DestDecl { dest_names = names
                               , dest_type  = ty
                               , dest_doc   = Nothing }
...
transCodata (L loc c@(CodataDecl {})) = unitOL dataDecl
                              `mappend` getters
                              `mappend` setters
  where dataDecl :: LHsDecl GhcPs
        dataDecl = L loc . TyClD noExt
                 $ DataDecl { tcdLName    = tccdLName c
                            , tcdDExt     = noExt
                            , tcdTyVars   = tccdTyVars c
                            , tcdFixity   = tccdFixity c
                            , tcdDataDefn = dataDefn }
        dataDefn = HsDataDefn { dd_ND      = DataType
                              , dd_ext     = noExt
                              , dd_ctxt    = cdd_ctxt . tccdCodataDefn $ c
                              , dd_cType   = Nothing
                              , dd_kindSig = Nothing
                              , dd_cons    = [noLoc record]
                              , dd_derivs  = noLoc [] }
        record = ConDeclH98 { con_name    = consName
                            , con_ext     = noExt
                            , con_forall  = noLoc False
                            , con_ex_tvs  = []
                            , con_mb_cxt  = Nothing
                            , con_args    = RecCon (noLoc fields)
                            , con_doc     = Nothing }
        fields :: [LConDeclField GhcPs]
        fields = fmap (\(L loc d) ->
                       L loc ( ConDeclField
                             { cd_fld_ext = noExt
                             , cd_fld_names = fmap (\n -> noLoc (FieldOcc noExt (mkDestName' n)))
                                                   (dest_names d)
                             , cd_fld_type  = (tyFunDom . getLHsInstDeclHead . dest_type $ d)
                             , cd_fld_doc = Nothing }))
                      (cdd_dests . tccdCodataDefn $ c)
        tyFunDom (L loc (HsFunTy _ _ d)) = d
        ...
        getters :: OrdList (LHsDecl GhcPs)
        getters = toOL . concatMap (\d -> [mkGetterType (unLoc d), mkGetterTerm (unLoc d)]) $ dests
        mkGetterType d = noLoc . SigD noExt
                       $ TypeSig noExt
                                 (fmap mkDestName (dest_names d))
                                 (mkHsWildCardBndrs . dest_type $ d)
        mkGetterTerm d = noLoc . ValD noExt . unLoc
                       $ mk_easy_FunBind
                           noSrcSpan
        ...
        setters :: OrdList (LHsDecl GhcPs)
        setters = toOL . reverse . mapWithIndex mkSetter . cdd_dests . tccdCodataDefn $ c
        mkSetter i (L _ d) = noLoc . ValD noExt . unLoc
                           $ mk_easy_FunBind
                               noSrcSpan
                               (unLoc . mkSetterName . head . dest_names $ d)
                               (fmap (noLoc . VarPat noExt . noLoc) [varX,varD])
                               (foldlWithIndex (\j c p ->
                                               ...
                                               dests)
transCodata (L loc _other) = unitOL . L loc . TyClD noExt $ _other
...
------------------------------------------
-- Cocase expression translation helpers
data Cocase = Cocase [(Copattern,LHsExpr GhcPs)]

data FCocase = FCocase (FCopattern, ExtendedHsExpr)
                       Default
...
data FCopattern
  = FQDest (Located RdrName)
  | FQPat (LPat GhcPs)
  | FQHead

data Default
  = DefExtExpr ExtendedHsExpr
  | DefEmpty

data ExtendedHsExpr
  = ExtExpr    (LHsExpr GhcPs)
  | ExtFCocase FCocase
...
unplugCopattern QHead           = (QHead,Nothing)
unplugCopattern (QDest h QHead) = (QHead, Just (FQDest h))
unplugCopattern (QPat QHead p)  = (QHead, Just (FQPat p))
unplugCopattern (QDest h q)     = let (q',m) = unplugCopattern q
                                  in  (QDest h q',m)
unplugCopattern (QPat q p)      = let (q',m) = unplugCopattern q
                                  in  (QPat q' p, m)

flattenCocase :: Cocase -> P ExtendedHsExpr
flattenCocase (Cocase coalts) =
  case coalts of
    [] -> return (ExtExpr hsUnmatchedPattern)

    -- R-QHead
    ((QHead,u):_) -> return (ExtExpr u)

    -- R-QDest
    ((QDest h QHead,u):coalts) ->
      ExtFCocase . FCocase (FQDest h,ExtExpr u) . DefExtExpr
        <$> flattenCocase (Cocase coalts)
    ...
    ((QPat QHead p,u):coalts) ->
      case unLoc p of
        ...
        VarPat _ _  -> return . ExtExpr $ mkHsLam [p] u
        ...
        _ -> do { coalts' <- flattenCocase (Cocase coalts)
                ...
                ; let matches = [ mkSimpleMatch CaseAlt [p] u
                                , mkSimpleMatch CaseAlt [noLoc . WildPat $ noExt ]
                                    (nlHsApp r (noLoc . HsVar noExt $ x))
                                ]
                ; return . ExtExpr $
                    mkHsLam [noLoc . VarPat noExt $ x]
                            (nlHsCase (noLoc . HsVar noExt $ x) matches)
                }
        ...
        (qrest,Just q') ->
         -- the sharing version
          do { coalts' <- flattenCocase (Cocase coalts)
             ...
             ; pv <- freshVar "p"
             ; let q'' = case q' of
                           FQPat p -> FQPat . noLoc $ AsPat noExt pv p
                           _ -> q'
             ; let def = case q' of
                           FQHead -> r
                           FQDest h -> nlHsApp (noLoc . HsVar noExt . mkDestName $ h)
                                               (noLoc (HsVar noExt v))
                           FQPat p -> nlHsApp (noLoc (HsVar noExt v)) (noLoc (HsVar noExt pv))
             ; u' <- flattenCocase (Cocase [ (qrest,u) , (QHead, def )])
             ; return
                 ( ExtLet v (ExtExpr r)
                 $ ExtFCocase
                 $ FCocase (q'',u') (DefExtExpr (ExtExpr (noLoc (HsVar noExt v))))
                 )
             }

transExtendedExpr :: ExtendedHsExpr -> P (LHsExpr GhcPs)
transExtendedExpr (ExtExpr u) = return u
transExtendedExpr (ExtFCocase (FCocase (fq,u) def)) =
  case fq of
    FQHead -> transExtendedExpr u
    FQDest h -> do { def' <- transDefault def
    ...
    FQPat  p -> do { def' <- transDefault def
                   ...
                   ; let matches = [mkSimpleMatch CaseAlt [p] u'
                                   ,mkSimpleMatch CaseAlt [noLoc . WildPat $ noExt]
                                                  (nlHsApp def' (noLoc . HsVar noExt $ x))]
                   ; return (mkHsLam [noLoc . VarPat noExt $ x]
                                     (nlHsCase (noLoc . HsVar noExt $ x) matches))
                   }
...
mkHsLet (L l v) a b =
   ...
   HsLet noExt (noLoc (HsValBinds noExt (ValBinds noExt (unitBag bind) []))) b
