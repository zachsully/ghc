...
mkConstructorName (L l s) = L l $
  case s of
    Unqual n -> Unqual (addMk n)
    Qual m n -> Qual m (addMk n)
    Orig m n -> Orig m (addMk n)
    Exact  n -> Exact  (n { n_occ = addMk (n_occ n) })
  where addMk n = n { occNameFS = appendFS (fsLit "Mk") (occNameFS n) }

-- these functions are generated by translating codata declarations
mkSetterName :: Located RdrName -> Located RdrName
mkSetterName (L l s) = L l $
  case s of
    Unqual n -> Unqual (addSet n)
    Qual m n -> Qual m (addSet n)
    Orig m n -> Orig m (addSet n)
